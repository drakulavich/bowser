...
- **`-h`/`--help` before `--` short-circuits in `run()` (`src/cli.ts`), after `parse` and before the command runs.** `<cmd> --help` prints `renderCommandHelp` (`src/cli/help.ts`), built from the registry like the general help; before this, `close --help` closed the session. `bowser mcp` is intercepted at the entry, above `run()`, so that branch checks `helpRequested` itself or `mcp --help` would start the server. `helpRequested` (`src/cli/parser.ts`) scans raw argv up to the first `--`, and `parse` skips unknown flags while help is asked for: a parse error must not hide `--help`, or `mcp --bogus --help` starts the server. `tests/help.test.ts` runs `--help` on every registered command with a `connect` that counts calls.
