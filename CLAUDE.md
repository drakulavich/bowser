...
- **A persistent profile lives in `~/.bowser/profiles/<session>/` (`profileDir()`), never under the session directory.** `close` does `rm -rf` of `sessionDir()`, so a profile there would be deleted by the very command it must survive. The daemon gets it from `open` through the `BOWSER_DAEMON_PROFILE` env var (not argv: `looksLikeOurDaemon` reads the last two argv words), which reaches both spawn paths; the daemon reports it in `state` so `open --persistent` can refuse a daemon running with another store. On close each engine must flush first: Chromium through CDP `Browser.close` plus a wait for its process (found with `ps` as our child with that `--user-data-dir`, since chrome-headless-shell writes no `SingletonLock`; one 1.5 s deadline bounds the wait and every `ps`, and aborting kills a stray one), WebKit by navigating to `about:blank` (`wrapView`'s `close`); without that, writes made just before `close` were lost.
- **Dialogs are answered the moment they open; never leave one pending.** A dialog left open on Chromium blocks the op that raised it and every op queued after it. The first design kept dialogs pending, like `playwright-cli`, and four review rounds kept finding races around the blocked call. Now every dialog is answered at once: with the one-shot answer from `dialog-accept`/`dialog-dismiss` if one is set, else dismissed. The report then goes into the daemon's queue, which is handed only to a request marked `report: true` (`withPageClient`). All of this lives in `createHandler` (`src/daemon/server.ts`).
  - **Chrome.** `answerDialog` answers the CDP dialog event and keeps the one-shot answer in `DaemonState.answer`. The answer is dropped when a main-frame navigation starts. The main frame is `Target.getTargetInfo`'s target id, not `Page.getFrameTree`, which a load-time dialog blocks. A landing (`onNavigated`) does not count on chrome, because Bun fires it for iframe landings too. A dialog is recorded only once `Page.handleJavaScriptDialog` resolves, in open order (the `answering` chain). A refused answer is retried once as a dismiss, else recorded as `state: "failed"`. Each answer attempt is bounded by `ANSWER_TIMEOUT_MS`, so the chain always settles and a reply waits for it. A timed-out answer may still land, so it is recorded `failed` / `answer timed out` at once. A bounded dismiss is then sent without waiting, so a dialog that really is open still closes. Its outcome never changes the report. A request that overruns `BOWSER_OP_TIMEOUT_MS` gets the reports queued so far on its timeout reply (`Handler.timedOut`). Its late reply takes none.
  - **WebKit** has no dialog events (`watchDialogs` returns false). There, `runShimmed` installs the page shim from `src/page-scripts.ts` (`dialogShim`, `withDialogShim`, `dialogSyncScript`, `dialogAnswerScript`). The shim replaces `window.alert/confirm/prompt` and keeps the one-shot answer and a report log on `window`.
  ...
  - Keep the two answer rules alike.
