...
if (import.meta.main) {
  ...
  if (process.argv[2] === "--daemon") {
    ...
    try {
      ...
      const userError = /^(usage:|unknown command|unknown flag|invalid --|expected a ref|ref '.*' not found|no open page|invalid BOWSER_BACKEND|BOWSER_BACKEND=webkit)/i.test(msg);
